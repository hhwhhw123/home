...
function modifypassword() {
    if (Validator.Validate(document.getElementById('demo'), 3)) {
        ...
        var params = $("demo").serialize(); //Form.serialize('updatapsd');
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: psdResponse,
            asynchronous: true
        });
    }
}
...
function modifyadmin() {
    if (Validator.Validate(document.getElementById('demo'), 3)) {
        ...
        var params = $("demo").serialize(); //Form.serialize('updaAdmin');
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: adminResponse,
            asynchronous: true
        });
    }
}
...
function addnotify() {
    if (Validator.Validate(document.getElementById('demo'), 3)) {
        ...
        var params = $("demo").serialize(); //Form.serialize('addNotify');
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: notifyResponse,
            asynchronous: true
        });
    }
}
...
function addHome() {
    if (Validator.Validate(document.getElementById('demo'), 3)) {
        ...
        var params = $("demo").serialize(); //Form.serialize('addHomes');
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: homesResponse,
            asynchronous: true
        });
    }
}
...
function searchhome() {
    ...
    var params = $("demo").serialize(); //Form.serialize('searchhomes');
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
        onComplete: searchhomesResponse,
        asynchronous: true
    });
}
...
function deletehomes() {
    if (selectedTr != null) {
        if (selectedTr.cells[6].childNodes[0].value == "已入住") {
            ...
            if (confirm("【该房产有业主】\n确定删除?")) {
                id = selectedTr.cells[0].getElementsByTagName("input")[0].value;
                number = selectedTr.cells[1].getElementsByTagName("input")[0].value
                var url = "deleteHomes.action?id=" + id + "&number=" + number;
                var myAjax = new Ajax.Request(url, {
                    method: 'post',
                    onComplete: deletehomesResponse,
                    asynchronous: true
                });
                selectedTr = null;
            }
...
function addperson() {
    if (Validator.Validate(document.getElementById('demo'), 3)) {
        ...
        //将表单中的数据系列化
        var params = $("demo").serialize(); //Form.serialize('addPersonnel');
        //向服务器发出请求
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: personnelResponse,
            asynchronous: true
        });
    }
}
...
function ruzhiperson() {
    if (selectedTr != null) {
        ...
        //将表单中的数据系列化
        var params = $("demo2").serialize(); //Form.serialize('ruzhiPersonnel');
        //向服务器发出请求
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
...
function lizhiperson() {
    if (selectedTr != null) {
        ...
        //将表单中的数据系列化
        var params = $("demo").serialize(); //Form.serialize('lizhiPersonnel');
        //向服务器发出请求
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
...
function updatanotify() {
    ...
    //将表单中的数据系列化
    var params = $("demo").serialize(); //Form.serialize('updataNotify');
    //向服务器发出请求
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
...
function checked(obj) {
    obj.style.backgroundColor = '#fbf8e9'; //把点到的那一行变希望的颜色; 
    if (selectedTr != null)
//    	alert(selectedTr.cells[0].getElementsByTagName("input")[0].value)
...
function modifynotify() {
    if (selectedTr != null) {
        var id = selectedTr.cells[0].getElementsByTagName("input")[0].value;
...
function deletemaintain() {
    ...
    if (selectedTr != null) {
        if (confirm("确定删除?")) {
            id = selectedTr.cells[0].getElementsByTagName("input")[0].value;
            var url = "deleteMaintain.action?p=" + id;
            var myAjax = new Ajax.Request(url, {
                method: 'post',
                onComplete: maintainResponse,
                asynchronous: true
...
function searchperson() {
    ...
    //将表单中的数据系列化
    var params = $("demo").serialize(); //Form.serialize('savesearchinfo');
    //向服务器发出请求
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
        onComplete: searchpersonnelResponse,
        asynchronous: true
    });
}
...
function addxuncha() {
    ...
    var params = $("demo").serialize(); //Form.serialize('addXuncha');
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
        onComplete: xunchaResponse,
        asynchronous: true
    });
}
...
function modifyxuncha() {
    ...
    //将表单中的数据系列化
    var params = $("demo").serialize(); //Form.serialize('modifyXuncha');
    //向服务器发出请求
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
...
function addlvhua() {
    ...
    var params = $("demo").serialize(); //Form.serialize('addLvhua');
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
        onComplete: lvhuaResponse,
        asynchronous: true
    });
}
...
function yanshoulvhua() {
    ...
    var params = $("demo").serialize(); //Form.serialize('yanshouLvhua');
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
        onComplete: lvhuaResponse,
...
function sendmaintain() { //选择要安排处理的信息
    if (selectedTr != null) {
        document.getElementById('dengji').style.display = '';
    } else alert("请选择要安排处理的信息");
}
...
function domaint() {
    if (selectedTr != null) {
        ...
        //将表单中的数据系列化
        var params = $("demo").serialize(); //Form.serialize('doMaintain');
        //向服务器发出请求
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: unmaintainResponse,
            asynchronous: true
        });
...
function dengjimaintain() { //选择要登记处理结果的信息
    if (selectedTr != null) {
        document.getElementById('jieguo').style.display = '';
    } else alert("请选择要登记处理结果的信息");
}
...
function resultmaint() {
    if (selectedTr != null) {
        ...
        //将表单中的数据系列化
        var params = $("demo").serialize(); //Form.serialize('resultMaintain');
        //向服务器发出请求
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: resultmaintainResponse,
            asynchronous: true
        });
...
function delnotify() {
    ...
    if (selectedTr != null) {
        if (confirm("确认删除?")) {
            s = selectedTr.cells[0].getElementsByTagName("input")[0].value;
            //设置执行的action并传值
...
function detailnotifytain(id) {
	getPart("detailNotify.action?id=" + id);
}
...
function modifyperson() {
    ...
    //将表单中的数据系列化
    var params = $("demo").serialize(); //Form.serialize('modifyPersonnel');
    //向服务器发出请求
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
...
function zhengshiruzhi() {
    var s = "";
    if (selectedTr != null) {
        s = selectedTr.cells[7].getElementsByTagName("input")[0].value
...
function yinchanghomes() {
    if (selectedTr != null) {
        if (selectedTr.cells[6].getElementsByTagName("input")[0].value == "已入住") {
            alert("该房产已有住户!");
        } else {
            document.getElementById("homes").style.display = "none";
            document.getElementById("addyezhu").style.display = "";
            document.getElementById("yezhu_homesid").value = selectedTr.cells[0].getElementsByTagName("input")[0].value;
            document.getElementById("yezhu_homesnumber").value = selectedTr.cells[1].getElementsByTagName("input")[0].value;
        }
    } else alert("请选择一个房产");
}
...
function addyezhu() {
    if (Validator.Validate(document.getElementById('demo2'), 3)) {
        var url = "addYezhu.action";
        var params = $("demo2").serialize(); //Form.serialize('addYezhu');
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: addyezhuResponse,
            asynchronous: true
        });
    }
}
...
function save_search_yezhu() {
    ...
    var params = $("demo").serialize(); //Form.serialize('save_yezhu_searchinfo');
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
        onComplete: searchyezhuResponse,
        asynchronous: true
    });
}
...
function deleteyezhu() {
    if (selectedTr != null) {
        if (confirm("确定删除住户\n把房产置空?")) {
            var id = selectedTr.cells[0].getElementsByTagName("input")[0].value;
            var number = selectedTr.cells[2].getElementsByTagName("input")[0].value
            var url = "delete_yezhu.action?id=" + id + "&number=" + number;
            var myAjax = new Ajax.Request(url, {
                method: 'post',
                onComplete: deleteyezhuResponse,
                asynchronous: true
            });
            selectedTr = null;
        }
...
function submit_modify_yezhu() {
    if (Validator.Validate(document.getElementById('demo'), 3)) {
        ...
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: modifyyezhuResponse,
            asynchronous: true
        });
    }
}
...
function querenfenpei() {
    if (Validator.Validate(document.getElementById('demo'), 3)) {
        ...
        var params = $("demo").serialize(); //Form.serialize('fenpeizhanghu');
        var myAjax = new Ajax.Request(url, {
            method: 'post',
            parameters: params,
            onComplete: fenpeiResponse,
            asynchronous: true
        });
        document.getElementById("fenpeizhanghu").style.display = "none";
    }
...
function addCar() {
    ...
    var params = $("demo").serialize(); //Form.serialize('addCars');
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
        onComplete: carsResponse,
        asynchronous: true
    });
}
...
function updataCar() {
    ...
    //将表单中的数据系列化
    var params = $("demo").serialize(); //Form.serialize('updataCars');
    //向服务器发出请求
    var myAjax = new Ajax.Request(url, {
        method: 'post',
        parameters: params,
