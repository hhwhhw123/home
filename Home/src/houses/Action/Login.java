...
public class Login extends ActionSupport {
	...
	public String checkLogin() {
		...
		// 验证管理员登录
		if (this.check.equals("admin")
				&& admin_account_service.loginCheck(this.loginname, this.pwd)) {
		...
		// 验证住户登录
		if (this.check.equals("custom")
				&& custom_account_service.customLoginCheck(this.loginname, this.pwd)) {
			request = ServletActionContext.getRequest();
			HttpSession sess = request.getSession();
